...
test("renders the viewer route at wide and constrained sizes", async ({
  page,
}) => {
  for (const width of [1920, 412]) {
    ...
    await expect(page.locator('a[href="/viewer"]').first()).toHaveAttribute(
      "aria-current",
      "page",
    );
    await expect(
      page.locator('a[href="/"]').filter({ hasText: "Status" }).first(),
    ).not.toHaveAttribute("aria-current", "page");
  }
});

test("shows the in-shell Page not found state without redirecting", async ({
  page,
}) => {
