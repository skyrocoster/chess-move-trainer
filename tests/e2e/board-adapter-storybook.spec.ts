...
const STORY_IDS = {
  starting: "board-adapter--default-valid-starting-position",
  rich: "board-adapter--rich-position",
  black: "board-adapter--black-orientation",
  hidden: "board-adapter--hidden-coordinates",
  constrained: "board-adapter--constrained-width",
  invalid: "board-adapter--invalid-fen",
  expanded: "board-adapter--expanded-position-description",
} as const;

async function openStory(page: Page, storyId: string) {
  await page.goto(`${STORYBOOK_URL}/iframe.html?id=${storyId}&viewMode=story`);
  await expect(page.locator('[class*="adapter"], [class*="unavailable"]').first()).toBeVisible();
...
test.describe("Board Adapter Storybook surface", () => {
  test("exercises all seven stories, static behavior, sizing, axe, and forced colors", async ({ page }) => {
    await page.emulateMedia({ forcedColors: "active" });

    await openStory(page, STORY_IDS.starting);
    await expect(page.getByRole("img", { name: "Starting position" })).toBeVisible();
    await expect(page.locator('[role="img"] [role], [role="img"] [tabindex]')).toHaveCount(0);
    await expect(page.locator('[aria-roledescription="draggable"]')).toHaveCount(0);
    ...
    const richDescription = page.locator(`#${await richGraphic.getAttribute("aria-describedby")}`);
    ...
    await expect(page.getByRole("img", { name: "Starting position from Black's side" })).toBeVisible();
    const blackGraphic = page.locator('[role="img"]');
    const blackDescription = page.locator(`#${await blackGraphic.getAttribute("aria-describedby")}`);
    await expect(blackDescription).toContainText("Orientation: Black at the bottom.");
    await checkA11y(page);
