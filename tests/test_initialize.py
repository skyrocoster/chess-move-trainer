...
def test_parameterized_initialization_updates_identity_and_is_safe(tmp_path: Path) -> None:
    ...
    shutil.copytree(ROOT, destination, ignore=shutil.ignore_patterns(".git", ".venv", "node_modules", "__pycache__"))
    (destination / "node_modules").mkdir()
    (destination / "unknown-ignored.txt").write_text("keep", encoding="utf-8")
    command = [
        powershell(), "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(destination / SCRIPT.name),
        "-ProjectSlug", "sample-project", "-DisplayTitle", "Sample Project",
        "-ProjectDescription", "A sample project.", "-DocsBrand", "Sample Project Docs",
        "-CleanGenerated", "-NonInteractive", "-KeepInitializer",
    ]
...
def test_noninteractive_requires_explicit_cleanup(tmp_path: Path) -> None:
    ...
    command = [
        powershell(), "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(destination / SCRIPT.name),
        "-ProjectSlug", "sample-project", "-DisplayTitle", "Sample Project",
        "-ProjectDescription", "A sample project.", "-DocsBrand", "Sample Project Docs", "-NonInteractive",
    ]
    result = subprocess.run(command, cwd=destination, capture_output=True, text=True, timeout=30)
