        ...
        try:
            with _open_connection(
                ...
                if position_id is None:
                    return _sparse_insight(request)
                experience, observed_moves, queue_state, preference = _read_position_data(
                    connection,
                    position_id,
                    request,
                )
...
def _sparse_insight(request: PositionInsightRequest) -> PositionInsight:
    return PositionInsight(
        fen=request.fen,
        trainer_color=request.trainer_color,
        as_of=request.as_of,
        opening=None,
        experience=PositionInsightExperience(0, 0),
        observed_moves=(),
...
def _read_position_data(
    ...
    tuple[PositionInsightObservedMove, ...],
    str | None,
    ...
    rows = connection.execute(
        text(
            """
            SELECT o.datasource_game_id, o.dgp_move_uci, g.dg_trainer_color
    ...
    moves: dict[str, tuple[set[int], int]] = {}
    for row in rows:
        game_id = row[0]
        move_uci = row[1]
        color = row[2]
        ...
        if move_uci is not None:
            move_games, occurrence_count = moves.setdefault(move_uci, (set(), 0))
            move_games.add(game_id)
            moves[move_uci] = (move_games, occurrence_count + 1)

    observed = tuple(
        PositionInsightObservedMove(
            move_uci=move_uci,
            distinct_game_count=len(move_games),
            occurrence_count=occurrence_count,
        )
        for move_uci, (move_games, occurrence_count) in sorted(
            moves.items(), key=lambda item: (-len(item[1][0]), item[0])
        )
    )
    experience = PositionInsightExperience(
        distinct_game_count=len(game_ids),
        occurrence_count=sum(
            1 for row in rows if row[2] == request.trainer_color
        ),
    )
    queue_state = _read_queue_state(connection, position_id)
    preference = _read_preference(connection, position_id, request.as_of)
    return experience, observed, queue_state, preference
