...
def test_removed_per_ply_route_is_unavailable(client: TestClient) -> None:
    valid_uuid = client.get(f"/api/games/{WHITE_GAME_UUID}/positions/0")
    negative_ply = client.get(f"/api/games/{WHITE_GAME_UUID}/positions/-1")
    malformed_uuid = client.get("/api/games/not-a-uuid/positions/0")

    for response in (valid_uuid, negative_ply, malformed_uuid):
...
def test_whole_game_request_does_not_modify_corpus_or_widen_cors(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    client: TestClient,
) -> None:
    ...
    response = client.get(
        f"/api/games/{WHITE_GAME_UUID}/positions",
