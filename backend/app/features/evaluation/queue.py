...
from .schema import EVALUATION_SCHEMA_VERSION, require_evaluation_schema

MAX_FEN_LENGTH = 128
MAX_OBSERVE_FENS = 64

_QUEUE_COLUMNS = (
    "position_key, fen, state, position, attempts, schema_version, enqueued_at, "
...
def _item_from_row(row: tuple[object, ...]) -> EvaluationQueueItem:
    return EvaluationQueueItem(
        ...
        position=int(row[3]),
        attempts=int(row[4]),
        enqueued_at=str(row[6]),
        started_at=None if row[7] is None else str(row[7]),
        finished_at=None if row[8] is None else str(row[8]),
        last_error_code=None if row[9] is None else str(row[9]),
        last_error_details=None if row[10] is None else str(row[10]),
    )
...
def enqueue(connection: sqlite3.Connection, fen: object) -> tuple[str, EvaluationQueueItem]:
    ...
    selected_key = position_key_from_fen(selected)
    outcome = "queued"
    ...
    def operation() -> None:
        ...
        row = connection.execute(
            f"SELECT {_QUEUE_COLUMNS} FROM evaluation_queue WHERE position_key = ?",
            (selected_key,),
        ).fetchone()
        if row is None:
            ...
            connection.execute(
                ...
                (selected_key, selected, position, EVALUATION_SCHEMA_VERSION, _now()),
            )
            outcome = "queued"
            item = _item_from_row(
                connection.execute(
        ...
        connection.execute(
            ...
            "last_error_code = NULL, last_error_details = NULL, fen = ? "
            "WHERE position_key = ?",
            (position, selected, selected_key),
        )
...
def claim_next(connection: sqlite3.Connection) -> EvaluationQueueItem | None:
    ...
    def operation() -> None:
        ...
        if row is None:
            return
        connection.execute(
            "UPDATE evaluation_queue SET state = 'running', started_at = ? WHERE position_key = ?",
            (_now(), row[0]),
        )
        item = _item_from_row(
            connection.execute(
...
def complete(connection: sqlite3.Connection, fen: object, result_fen: object) -> None:
    ...
    selected_key = position_key_from_fen(selected)
    selected_result_key = position_key_from_fen(selected_result)
    if selected_result_key != selected_key:
        raise EvaluationValidationError("result FEN does not match the queued item")

    def operation() -> None:
        row = connection.execute(
            "SELECT state FROM evaluation_queue WHERE position_key = ?", (selected_key,)
        ).fetchone()
        if row is None or row[0] != "running":
            raise EvaluationQueueError("cannot complete an item that is not running")
        connection.execute(
            "UPDATE evaluation_queue SET state = 'done', finished_at = ?, fen = ? "
            "WHERE position_key = ?",
            (_now(), selected_result, selected_key),
        )
...
def fail(
    ...
    def operation() -> None:
        ...
        connection.execute(
            ...
            "last_error_code = ?, last_error_details = ?, fen = ? WHERE position_key = ?",
            (_now(), error_code[:200], details[:2000], selected, selected_key),
        )
...
def requeue_running(connection: sqlite3.Connection) -> int:
    ...
    def operation() -> None:
        ...
        rows = connection.execute(
            "SELECT position_key FROM evaluation_queue WHERE state = 'running' "
            "ORDER BY position ASC"
        ).fetchall()
        base = _next_position(connection)
        for index, (position_key,) in enumerate(rows):
            connection.execute(
                "UPDATE evaluation_queue SET state = 'queued', position = ?, "
                "started_at = NULL, finished_at = NULL, "
                "last_error_code = NULL, last_error_details = NULL WHERE position_key = ?",
                (base + index, position_key),
            )
            count += 1
...
def observe(connection: sqlite3.Connection, fen: object) -> EvaluationQueueItem | None:
    ...
    selected_key = position_key_from_fen(selected)
    row = connection.execute(
        f"SELECT {_QUEUE_COLUMNS} FROM evaluation_queue WHERE position_key = ?", (selected_key,)
    ).fetchone()
...
def observe_many(
    ...
    selected_keys = tuple(position_key_from_fen(fen) for fen in selected)
    found: dict[PositionKey, EvaluationQueueItem] = {}
    for start in range(0, len(selected), 100):
        chunk = tuple(selected[start : start + 100])
        placeholders = ",".join("?" for _ in chunk)
        key_chunk = tuple(selected_keys[start : start + 100])
        rows = connection.execute(
            f"SELECT {_QUEUE_COLUMNS} FROM evaluation_queue WHERE position_key IN ({placeholders})",
            key_chunk,
        ).fetchall()
        for row in rows:
            item = _item_from_row(row)
            found[item.position_key] = item
    return {fen: found.get(key) for fen, key in zip(selected, selected_keys)}
